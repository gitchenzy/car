...
class EmployeeController extends AdminController
{
    ...
    public function loadEmployee(){
        ...
        $count = $this->emp -> where($where) -> order('Sort desc') -> count();
        $res = $this->emp -> where($where) -> limit($offset,$limit) -> order('Sort desc') -> select();
        foreach($res as &$v){
            ...
            $v['SexName'] = $v['Sex']==1?'男':'女';
            $v['StatusName'] = $v['Status']==1?'锁定':'正常';
            $v['Priority'] = $v['isPriority']==1?'是':'否';
        }
    ...
    public function addEmployee(){
        if(IS_POST){
            ...
            $res = $this -> emp -> where(array('EmployeeNum' =>$data['EmployeeNum'],'DelFlag'=>0)) -> find();
            if($res){
            ...
            if($result){
                ...
                $this->emp -> where(array('EmployeeID'=>$result,'DelFlag'=>0)) -> save($sa);
    ...
    public function editEmployee(){
        ...
        if(IS_POST){
            ...
            // dump($data['duty']);
            $this -> emp ->startTrans();
            $res = $this -> emp -> where(array('DelFlag'=>0,'EmployeeNum' =>$data['EmployeeNum'],'EmployeeID'=>array('neq',$id))) -> find();
            if($res){
                $this->error('该账号已经有人使用，请输入新的账号！');
            }
            if(isset($data['duty']) && !empty($data['duty'])){
            ...
            //在修改之后先判断是否改掉的是ADMIN
            $resu = $this -> emp -> where(array('DelFlag'=>0,'EmployeeNum' =>'admin')) -> find();
            if(!$resu){
                $this -> emp ->rollback();
                $this->error('该账号不能被修改！');
            }else{
